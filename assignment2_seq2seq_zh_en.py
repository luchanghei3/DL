...
class GPT2BPETokenizerAdapter:
    """Optional GPT-2 BPE adapter for English text.

    It converts token ids back to GPT-2 token strings so a task-specific vocabulary
    can be built on top of stable subword pieces.
    """
    ...
    def __call__(self, text: str) -> List[str]:
        token_ids = self.encoder.encode(normalize_english(text))
...
def build_examples(
    ...
    english_tokenizer: Callable[[str], List[str]],
    max_source_len: int | None = None,
    max_target_len: int | None = None,
) -> list[Example]:
    examples = []
    for source_text, target_text in pairs:
        source_tokens = tokenize_chinese(source_text)
        target_tokens = english_tokenizer(target_text)
...
def choose_english_tokenizer(mode: str) -> Callable[[str], List[str]]:
    if mode == "word":
        return tokenize_english_words
    if mode == "bpe":
        try:
            return GPT2BPETokenizerAdapter()
        except Exception as exc:
            print(f"Falling back to word tokenizer because GPT-2 BPE setup failed: {exc}")
            return tokenize_english_words
    raise ValueError(f"Unsupported tokenizer mode: {mode}")


def main() -> None:
    parser = argparse.ArgumentParser(description="Chinese-to-English seq2seq with attention for Assignment 2 questions 1-2.")
    parser.add_argument("--data-path", type=Path, required=True, help="Path to the extracted cmn.txt-style parallel corpus.")
    parser.add_argument("--tokenizer", choices=["word", "bpe"], default="word", help="English tokenization strategy.")
    ...
    english_tokenizer = choose_english_tokenizer(args.tokenizer)
    pairs = read_parallel_corpus(args.data_path)
    if args.limit is not None:
        pairs = pairs[: args.limit]
    examples = build_examples(pairs, english_tokenizer, args.max_source_len, args.max_target_len)
    ...
    report_lines = [
        ...
        f"- English tokenizer: `{args.tokenizer}`",
